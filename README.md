...
## 镜像目录 / Mirrored Catalog

本仓库的 `v1-mirror.json` 由 GitHub Actions 定时从上游 `v1.json` 同步生成：把每个主题的 `download` 和 `preview` 地址套上镜像前缀（ghproxy 类透传代理），`sha256`、`url` 等字段保持原样。镜像代理原样透传文件字节，因此 `sha256` 校验仍然有效。

The `v1-mirror.json` in this repository is generated by a scheduled GitHub Action from the upstream `v1.json`: it prefixes every theme's `download` and `preview` URL with a mirror proxy (a ghproxy-style pass-through proxy), leaving `sha256`, `url` and other fields untouched. Because the proxy relays bytes unchanged, the `sha256` checksums remain valid.

在 Komari 中把市场源指向镜像目录，即可在不访问 GitHub 的情况下浏览与安装主题：

Point a Komari theme market source at the mirrored catalog to browse and install themes without direct GitHub access:

```text
https://gh-proxy.com/https://raw.githubusercontent.com/CelestNya/komari-theme-market/main/v1-mirror.json
```

同步任务每六小时运行一次（分钟 43 分），也可在 Actions 页面手动触发 `Sync Mirror Catalog`。更换镜像代理时，在仓库 Settings → Variables 里设置 `MIRROR_PREFIX`（例如 `https://ghfast.top/`），未设置时使用默认 `https://gh-proxy.com/`。

The sync job runs every six hours (at minute 43) and can be triggered manually via the `Sync Mirror Catalog` workflow. To switch mirror proxies, set the `MIRROR_PREFIX` repository variable (for example `https://ghfast.top/`); it defaults to `https://gh-proxy.com/` when unset.
